...
with st.form("production_form", clear_on_submit=True):
    ...
    if st.form_submit_button("🚀 Log Production & Sync"):
        if not job_code or not worker:
            st.warning("Please fill in Job Code and Worker Name.")
        else:
            ...
            if save_to_github(df):
                st.success(f"✅ Production for {job_code} synced to GitHub!")
                st.rerun()
...
if not df.empty:
    # Sort by time so latest is on top

...
# --- 7. NEW: LOG SUMMARY (KPIs) ---
st.divider()
st.subheader("📊 Shopfloor Summary")
if not df.empty:
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Total Logs", len(df))
    m2.metric("Total Hours", f"{df['Hours'].sum():.1f} Hrs")
    m3.metric("Active Jobs", df['Job_Code'].nunique())
    m4.metric("Active Workers", df['Worker'].nunique())
